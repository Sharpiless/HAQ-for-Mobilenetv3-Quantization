...
model_urls = {
    'mobilenet': 'https://hanlab.mit.edu/files/haq/mobilenetv3small-f3be529c.pth',
}
...
class MobileNet(nn.Module):
    def __init__(self, num_classes=1000, conv_layer=nn.Conv2d, profile='normal', w_mul=1.):
        ...
        if conv_layer is not nn.Conv2d and w_mul == 2.:
            in_planes = 64
            cfg = [128, (256, 2), 256, (512, 2), 512, (1024, 2),
                   1024, 1024, 1024, 1024, 1024, (2048, 2), 2048]

        if conv_layer == nn.Conv2d:
            self.conv1 = conv_bn(3, in_planes, stride=2)
        else:
            self.conv1 = conv_bn(3, in_planes, stride=2,
                                 conv_layer=conv_layer, half_wave=False)
    ...
    def _make_layers(self, in_planes, cfg, conv_layer):
        ...
        for x in cfg:
            ...
            layers.append(conv_dw(in_planes, out_planes,
                          stride, conv_layer=conv_layer))
